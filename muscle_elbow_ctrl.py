...
def main():
    ...
    with tqdm(total=SIMLEN*1.001) as pbar:
        while data.time - simstart < SIMLEN:
            ...
            # apply ctrl
            req_torque = P_control(curr_angle, req_angle)
            muscle_name="BRA"

            #data.ctrl[model.actuator(muscle_name).id] = get_single_muscle_activation(muscle_name, curr_angle, req_torque)
    ...
    record.plot_data(elbow_angle_series, "muscle_ctrl/ol_elbow_angle", title="Elbow Angle [degrees]")
    record.plot_data(torque_series, "muscle_ctrl/ol_joint_torque", title="Torque generated by muscle activation [Nm]",ref_series=torque_ref_series)

    for j in range(len(muscle_input_series[0][1])):
        input_series_j = [(t, inputs[j]) for t, inputs in muscle_input_series]
        record.plot_data(input_series_j, f"muscle_ctrl/ol_muscle_input_{j}", muscle_names[j])
    print("Plots saved to ./plots/muscle_ctrl/ folder")
